...
static void light_ctrl_config_task(void *arg);
static void light_ctrl_task(void *arg);
static void IRAM_ATTR light_toggle_isr(void *arg);

static SCHEDULER_TaskTypeDef gLightCtrlCfgTask = {
    .Active = 0,
    .CoreID = LIGHT_CONTROL_CFG_TASK_CORE_ID,
    .Name = "Light control config task",
    .Priority = LIGHT_CONTROL_CFG_TASK_PRIORITY,
    .StackDepth = LIGHT_CONTROL_CFG_TASK_STACK_DEPTH,
    .Function = light_ctrl_config_task
};

void LCTRL_Init() {
    SCHEDULER_Create(&gLightCtrlCfgTask);
}

void light_ctrl_config_task(void *arg) {
    esp_err_t esp_err;
    ...
    if ((esp_err = gpio_config(&io_cfg)) != ESP_OK) {
        LOGGER_LogF(LOGGER_LEVEL_FATAL, "Failed to initialize light control GPIO! Error code: %d", esp_err);
    } else {
    ...
    if ((esp_err = gpio_config(&io_cfg)) != ESP_OK) {
        LOGGER_LogF(LOGGER_LEVEL_FATAL, "Failed to initialize light toggle GPIO! Error code: %d", esp_err);
    } else {
        LOGGER_Log(LOGGER_LEVEL_INFO, "Light toggle GPIO initialized successfully!");
    };

    if ((esp_err = gpio_install_isr_service(0)) != ESP_OK) {
        LOGGER_LogF(LOGGER_LEVEL_FATAL, "Failed to install GPIO ISR service! Error code: %d", esp_err);
    };

    if ((esp_err = gpio_isr_handler_add(LIGHT_TOGGLE_GPIO, light_toggle_isr, NULL)) != ESP_OK) {
        LOGGER_LogF(LOGGER_LEVEL_FATAL, "Failed to add light toggle ISR handler! Error code: %d", esp_err);
    };
    ...
    SCHEDULER_Remove(&gLightCtrlCfgTask);
...
void light_ctrl_task(void *arg) {
    while (1) {
        ...
        if (xTaskNotifyWait(0, 0xFF, &enable, portMAX_DELAY)) {
            // Enable light
            if (enable == 1) gpio_set_level(LIGHT_CTRL_GPIO, 1);
            // Toggle light
            else if (enable == 2) gpio_set_level(LIGHT_CTRL_GPIO, !gpio_get_level(LIGHT_CTRL_GPIO));
            // Disable light
            else gpio_set_level(LIGHT_CTRL_GPIO, 0);
            LOGGER_Log(LOGGER_LEVEL_INFO, enable == 1 ? "Light enabled" : "Light disabled");
        }
    }
}
